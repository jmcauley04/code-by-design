...
export function generateFileContent(node: FileNode, language: string, framework: string): string {
  ...
  const template = templates.find(
    ...
    t => t.language === language && t.framework === 'any' && t.fileType === 'unknown'
  ) || templates[templates.length - 1];

  return template.generate(node);
}

export function generateProject(req: GenerateRequest): Record<string, string> {
  const result: Record<string, string> = {};

  for (const file of req.files) {
    if (file.type === 'style' || file.type === 'config') continue;
...
function generateIndexFile(req: GenerateRequest): string {
